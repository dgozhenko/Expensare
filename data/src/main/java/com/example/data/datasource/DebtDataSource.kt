...
class DebtDataSource @Inject constructor(private val database: ExpensareDatabase, private val storage: Storage): DebtInterface {
    override suspend fun create(debt: UserDebt) {
        val groupId = storage.groupId
        var operationDone = false
        // Get all firebase references
        val groupDebts = FirebaseDatabase.getInstance("https://expensare-default-rtdb.europe-west1.firebasedatabase.app/")
                .getReference("group_debts/$groupId")
                .push()

        val groupDebtsCheck = FirebaseDatabase.getInstance("https://expensare-default-rtdb.europe-west1.firebasedatabase.app/")
                .getReference("group_debts/$groupId/")

        val groupDebtsAdd = FirebaseDatabase.getInstance("https://expensare-default-rtdb.europe-west1.firebasedatabase.app/")
                .getReference("group_debts/$groupId")
        ...
        if (check.result!!.exists()) {
            check.result!!.children.forEach {
                ...
                if (userDebt.firstUser == debt.firstUser && userDebt.secondUser == debt.secondUser) {
                    operationDone = true
                    groupDebtsAdd.child("/$key/").setValue(UserDebt(
                        userDebt.firstUser,
                        userDebt.secondUser,
            ...
            if (!operationDone) {
                groupDebts.setValue(
                    UserDebt(debt.firstUser, debt.secondUser, debt.firstUserAmount, -debt.firstUserAmount, false)
                )
            }
        } else {
            groupDebts.setValue(
